                                             ...
                                             uint32_t multiboot_address)
{
    ...
    {
        auto entry = reinterpret_cast<Multiboot2MemoryMapEntry *>(reinterpret_cast<uint8_t *>(memory_map->entries) +
...
PhysicalAddress PhysicalMemoryManager::allocate()
{
    for (uint64_t i = 0; i < pages_; i++)
    {
        if (!test_bit(i))
        {
            set_bit(i);
            free_pages_--;
            return PhysicalAddress{base_ + i * PAGE_SIZE};
        }
    }
