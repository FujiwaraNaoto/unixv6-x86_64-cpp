...
{   
    ...
    void *Heap::sbrk(intptr_t increment)
    {
        ...
        if(increment < 0) return (void*)-1; // 負の増分はサポートしない
        if(increment == 0) return reinterpret_cast<void*>(brk_); // 現在のブレークポイントを返す
        ...
        for(Block *block = head_; block != nullptr; block = block->next) {
            if(block->free && block->size >= total_size) {
                ...
                if(block->size >= total_size + BLOCK_HEADER + MIN_SPLIT_SIZE) {
                    // ブロックを分割
                    Block *new_block = reinterpret_cast<Block*>(reinterpret_cast<uint8_t*>(block) + BLOCK_HEADER + total_size);
