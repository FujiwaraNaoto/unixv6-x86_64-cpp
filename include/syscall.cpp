...
static uint64_t sys_write(uint64_t fd, uint64_t buf, uint64_t len) {
    if (fd != 1 && fd != 2) return (uint64_t)-1;  // stdout/stderr のみ
    const char *p = reinterpret_cast<const char *>(buf);
    for (uint64_t i = 0; i < len; i++) vga::vga->putchar(p[i]);
    return len;
}
...
extern "C" uint64_t syscall_dispatch(
    ...
    switch (num) {
        case SyscallNo::kWrite: return sys_write(a1, a2, a3);
        case SyscallNo::kExit:  return sys_exit(a1);
        default:
